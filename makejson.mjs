...
for(let r of forks){
  try{
    ...
    let p=d.parent;
    results.push({fork:d.name,owner:p.owner.login,name:p.name,branch:p.default_branch,stars:p.stargazers_count,date:p.created_at});
...
for(let repo of repos){
  ...
  }catch(e){ if(e===404){ console.log("404 skipped:",repo.owner+"/"+repo.name,"("+repo.branch+")"); continue; } throw e; }
}
await printApiCalls();
}
...
let u = "unzips";
await run("mkdir -p '"+u+"'");
await run("bash -c 'for f in zips/*.zip; do name=$(basename \"$f\" .zip); unzip -o \"$f\" -d \""+u+"/$name\"; done'");
...
async function printApiCalls(){
let r=await octokit.request("GET /rate_limit");
console.log("API calls remaining:",r.data.rate.remaining);
}
...
https.get(url,res=>{
 if(res.statusCode===302||res.statusCode===301){ return download(res.headers.location,dest).then(resolve).catch(reject); } if(res.statusCode!==200)return reject(res.statusCode);
 fs.mkdirSync(path.dirname(dest),{recursive:true});
 let f=fs.createWriteStream(dest);
 res.pipe(f);
